...
try {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (v) {
      ...
      html += '<button onclick="location.href=\\'https://umalaaa.github.io/qx-data/delete?key=' + k + '\\'">Delete</button></div>';
    } else {
  ...
  if (bv) {
    ...
    html += '<button onclick="location.href=\\'https://umalaaa.github.io/qx-data/delete?key=RESP_barventory\\'">Delete All</button>';
  } else {
    html += '<p>Empty</p>';
  }
  html += '</div>';

  html += '<div style="margin-top:20px"><button onclick="location.href=\\'https://umalaaa.github.io/qx-data/clear\\'" style="background:red;width:100%;padding:10px">CLEAR ALL</button></div>';
  html += '</body></html>';
