...
keys.forEach((k, i) => {
  ...
  cards += '<div class="c"><div class="ch"><div><b>' + k.label + '</b><br><small>' + k.key + '</small></div><span class="s ' + (has?"g":"r") + '">' + (has?"有":"空") + '</span></div><div class="cb">' + (has ? '<pre>' + esc + '</pre><div class="br"><button onclick="cp(\\''+k.key+'\\')">复制Key</button><button onclick="cp(\\''+vJs+'\\')">复制Value</button><button class="d" onclick="location.href=\\'https://umalaaa.github.io/qx-data/delete?key='+k.key+'\\'">删除</button></div>' : '<p class="e">暂无数据</p>') + '</div></div>';
});
